...
def fetch_gdelt_news(
    ...
    """
    Fetch latest disaster-related news from GDELT.
    """

    import time

    logger.info(
        f"GDELT: Fetching news for query "
        f"'{disaster_query}' "
        f"(timespan: {timespan}, max: {max_records})"
    )

    base_url = "https://api.gdeltproject.org/api/v2/doc/doc"
    query_terms = disaster_query.split()

    formatted_query = " AND ".join(query_terms)

    formatted_query += " sourcelang:english"
    # Better query structure
    #formatted_query = f'"{disaster_query}" sourcelang:english'
    ...
    try:
        ...
        for attempt in range(3):
            ...
            logger.info(
                f"GDELT: Status code = {response.status_code}"
            )
        ...
        if response is None or response.status_code == 429:
            ...
            logger.error(
                "GDELT: Failed after retries due to rate limiting"
            )

            return {
                "error": "GDELT rate limited",
        ...
        if not response.text.strip():
            ...
            logger.error("GDELT: Empty response body")

            return {
                "error": "Empty response from GDELT",
                "query": disaster_query,
                "source": "GDELT",
                "articles": [],
            }
        ...
        if "<html" in response.text.lower():
            ...
            logger.error(
                "GDELT: Returned HTML instead of JSON"
            )

            logger.debug(response.text[:500])

            return {
                "error": "GDELT returned HTML",
                "query": disaster_query,
                "source": "GDELT",
                "articles": [],
            }
        ...
        logger.info(
            f"GDELT: Retrieved {len(articles)} articles"
        )
        ...
        logger.info(
            f"GDELT: Successfully processed "
        ...
        logger.error(
            f"GDELT: API request failed - {str(e)}"
        )
        ...
        logger.error(
            f"GDELT: JSON parsing failed - {str(e)}"
        )

        return {
            "error": f"GDELT JSON parsing failed: {str(e)}",
            "query": disaster_query,
            "source": "GDELT",
            "articles": [],
        }
        ...
        logger.error(
            f"GDELT: Unexpected error - {str(e)}"
        )

        return {
            "error": f"GDELT unexpected error: {str(e)}",
            "query": disaster_query,
            "source": "GDELT",
            "articles": [],
        }
...
def fetch_combined_disaster_news(disaster_query: str) -> dict:
    ...
    gdelt_data = fetch_gdelt_news(disaster_query, timespan="7d", max_records=10)
